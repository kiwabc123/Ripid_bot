 ...
 * <p>
 * This class is multithread safe. It is always <a href=
 * "http://www.ibm.com/developerworks/java/library/j-jtp0618.html">properly
 * constructed</a>. All of its fields are final. Its {@link #key} field is
 * immutable, while its {@link #client} field is a multithread safe type. See p.
 * 53 of <a href="http://www.javaconcurrencyinpractice.com">Java Concurrency In
...
public class REST_methods implements Closeable {
	...
	// OkHttp performs best when you create a single OkHttpClient instance and reuse
	// it for all of your HTTP calls.
	// This is because each client holds its own connection pool and thread pools.
	// Reusing connections and threads reduces latency and saves memory.
	// Conversely, creating a client for each request wastes resources on idle
	// pools.
	...
	@Override
	public void close() {
		// The code below is described in the "Shutdown isn't necessary" section here:
		// https://square.github.io/okhttp/3.x/okhttp/okhttp3/OkHttpClient.html
		client.dispatcher().executorService().shutdown();
		client.connectionPool().evictAll();
		// client.cache().close(); // only call this if client was created with a cache;
		// the one above was NOT
	}
	...
	public Asset[] list_all_assets() throws IOException {
		...
		for (int i = 0; i < array.length(); i++) {
			...
			System.out.println(result[i] + " " + asset_id);
		}

		return result;
	}

	public Asset[] list_by_id_assets() throws IOException {
		String json = get_json("/v1/assets/BTC");
		JSONArray array = new JSONArray(json);
	...
	public Symbol[] list_all_symbols() throws IOException {
		String json = get_json("/v1/symbols?filter_symbol_id=BINANCE");
		JSONArray array = new JSONArray(json);
	...
	public Exchange_rate get_exchange_rate(String asset_id_base, String asset_id_quote, Instant time)
			throws IOException {
	...
	public Timedata[] ohlcv_get_historical_timeseries(String symbol_id, Period_identifier period_id, Instant time_start)
			throws IOException {
	...
	public Timedata[] ohlcv_get_historical_timeseries(String symbol_id, Period_identifier period_id, Instant time_start,
		...
		String json = get_json("/v1/ohlcv/" + symbol_id + "/history?period_id=" + period_id + "&time_start="
	...
	public Orderbook[] orderbooks_get_historical_data(String symbol_id, Instant time_start, int limit)
			throws IOException {
	...
	public Orderbook[] orderbooks_get_historical_data(String symbol_id, Instant time_start, Instant time_end)
			throws IOException {
		String json = get_json(
				"/v1/orderbooks/" + symbol_id + "/history?time_start=" + time_start + "&time_end=" + time_end);
		JSONArray array = new JSONArray(json);
		return parse_orderbooks(array);
	}

	public Orderbook[] orderbooks_get_historical_data(String symbol_id, Instant time_start, Instant time_end, int limit)
			throws IOException {
		String json = get_json("/v1/orderbooks/" + symbol_id + "/history?time_start=" + time_start + "&time_end="
